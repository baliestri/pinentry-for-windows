...
When a smart card or hardware token (YubiKey, OpenPGP card, Nitrokey, or any CCID-compatible device) is in use, `gpg-agent` sends `SETKEYINFO --clear` before requesting the card PIN.
This instructs Pinentry For Windows to skip all cache operations for that prompt.
Windows Hello will not appear.

The card PIN is not stored in Windows `PasswordVault`.
Card authentication and PIN caching are managed internally by `gpg-agent` and its `scdaemon` component.
The hardware token itself enforces PIN retry limits.

The Windows Hello external cache applies only to software keys protected by a passphrase stored in a keyring file.
To verify which path is active for a given operation, check `%LOCALAPPDATA%\PinentryForWindows\pinentry.log` and look for `SETKEYINFO` — a value of `--clear` means caching is disabled for that `GETPIN` call.
